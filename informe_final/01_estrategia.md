...
## 6. Guion de demostración en vivo

La demostración ejecuta una venta real en el POS y muestra la propagación automática del evento a ERP, SCM y Dashboard. El flujo no requiere editar archivos ni ejecutar módulos por separado.
...
```powershell
cd D:\farmacia_salud
& .\venv\Scripts\python.exe .\farmacia_salud\main.py
```

El **Paso 1** carga los CSV reducidos en SQLite y reinicia la base de demostración. Esperar a que termine la carga inicial antes de continuar.

### Transacción y evidencia

En el **Paso 2**, el programa selecciona automáticamente una sucursal, un cliente y un producto, y registra una venta de 5 unidades. Antes de confirmar imprime `[ANTES]` con caja, stock y órdenes de compra. Después de confirmar imprime `[DESPUES]`, donde se debe verificar:

1. **ERP**: aparece `[ERP] Asiento generado` y aumenta el saldo de caja de la sucursal por el total de la venta.
2. **SCM**: aparece `[SCM] Stock OK` o `[SCM] Orden de compra ... generada`; en ambos casos el stock disminuye. Si queda por debajo del mínimo, se evidencia además la orden de compra automática.
3. **Dashboard**: antes de vender aparece `[Dashboard] Estado ANTES guardado en ...dashboard_antes.png`. Después de ERP y SCM aparece `[Dashboard] Actualizado automaticamente por INV... | ventas=... | total=... | archivo=...dashboard_despues.png`. El callback vuelve a consultar SQLite, muestra el nuevo conteo y total acumulado, y guarda el Dashboard posterior.

### Archivos que se deben mostrar

Al finalizar el Paso 2, abrir lado a lado `outputs/dashboard_antes.png` y `outputs/dashboard_despues.png`. El segundo incorpora la nueva venta en la evolución semanal y refleja el nuevo saldo ERP y las órdenes de compra SCM. También se puede mostrar `outputs/dashboard_completo.png`, `outputs/01_evolucion_ventas.png`, `outputs/03_kpi_erp_scm.png` y `data/farmacia.db` como evidencia de persistencia.

### Mensaje de cierre para la presentación

"Una única venta registrada en el POS dispara el evento de negocio: ERP registra el ingreso y actualiza caja, SCM descuenta inventario y evalúa reposición, y el Dashboard vuelve a consultar la base y actualiza sus indicadores sin intervención manual."

## 7. Análisis comparativo de los gráficos

La comparación visual debe hacerse entre `outputs/dashboard_antes.png` y `outputs/dashboard_despues.png`. La evidencia observada en la ejecución de la demo es:

| Indicador | Antes | Después | Evidencia del cambio |
|---|---:|---:|---|
| Ventas registradas | 6020 | 6021 | Se incorpora la venta `INV00006021`. |
| Caja chica, sucursal 1 | $0.00 | $34.20 | Aparece una barra azul en el gráfico **KPI ERP/SCM**. |
| Stock del producto demo | 90 | 85 | SCM descuenta las 5 unidades vendidas. |
| Órdenes de compra | 0 | 0 | No se genera OC porque el stock posterior todavía supera el mínimo. |
| Total acumulado de ventas | Sin la venta demo | Incluye $34.20 adicionales | El mensaje del Dashboard muestra el nuevo `total`. |

### Qué cambia en cada gráfico

- **KPI ERP/SCM**: es la diferencia más clara. En `dashboard_antes.png` la caja de la sucursal está en cero; en `dashboard_despues.png` aparece la barra azul de `$34.20`. Este gráfico evidencia la actualización ERP y, si se genera una orden, también la actualización SCM.
- **Evolución semanal de ventas**: el gráfico posterior incorpora la venta registrada en la fecha actual. La línea puede mostrar períodos intermedios con valor cero porque los datos históricos terminan antes de la fecha de la demo; esos ceros representan semanas sin ventas cargadas, no un fallo de actualización.
- **Segmentación RFM**: puede permanecer visualmente igual en esta transacción porque una sola venta no necesariamente cambia de segmento a una cantidad significativa de clientes. El RFM se recalcula, pero el cambio principal de esta demostración se evidencia en ventas y en el KPI ERP/SCM.

Por lo tanto, durante la presentación se debe resaltar principalmente el gráfico **KPI ERP/SCM**, acompañándolo con la comparación `[ANTES]`/`[DESPUES]` de caja y stock y con el aumento del contador de ventas mostrado por el callback del Dashboard.
