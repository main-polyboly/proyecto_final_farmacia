...
def guardar_snapshot_dashboard(nombre):
    origen = os.path.join(OUTPUT_DIR, "dashboard_completo.png")
...
def generar_dashboard_completo(session, rfm_df=None):
    ...
    plt.close(fig)
    print("[Dashboard] Graficos regenerados: outputs/dashboard_completo.png")


def procesar_venta_dashboard(evento):
    """Regenera las métricas del Dashboard cuando se confirma una venta."""
    ...
    try:
        ...
        dashboard_despues = guardar_snapshot_dashboard("dashboard_despues.png")
        ventas = session.query(Venta).all()
        total_ventas = sum(float(venta.total or 0) for venta in ventas)
        print(
            f"[Dashboard] ACTUALIZADO por {evento.venta.invoice} | "
            f"ventas={len(ventas)} | total acumulado=${total_ventas:.2f} | "
            "archivo=outputs/dashboard_despues.png"
        )
    finally:
        session.close()
